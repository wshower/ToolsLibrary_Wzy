...
public class SuperCustomToast {
    ...
    /**
     * 添加了if (!msg.equals(lastMsg))，如果消息不同的情况，
     * 原版没有重置firstCalled，如果msg不同，调用showSameMsg方法竟然就不显示后面的消息了，，坑！
     *
     * create by wzy (2016/6/18  15:02)
    */
    public void showSameMsg(String msg) {
        if (!msg.equals(lastMsg)) {
            firstCalled = true;
        }
        if (!TextUtils.isEmpty(msg)) {
            this.showSameMsg(msg,2500);
        }
    }
...
//        else tv.setBackgroundDrawable(background);
        ...
        tv.setBackground(mContext.getResources().getDrawable(R.drawable.bg_supercustomtoast_black_small));
